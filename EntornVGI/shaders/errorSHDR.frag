...
// --- L86- Transparència
    ...
    float aValue = 0.0;			        // Valor de coordenada a de transparència acumulat dels materials. 

// ---- L90- Compute emissive term
...
// ---- L110- Multiples llums
	...
	if (LightSource[i].sw_light) { 
		...
		// ---- L129- Compute Spot Light Factor
		...
		//-153- Compute the specular term
    		...
    		if (sw_intensity[3]) {	R = normalize (-reflect(L,N));
    					...
    					if (sw_material) { Ispecular = material.specular.rgb * LightSource[i].specular.rgb * specularLight * fatt;
                                                           // Càlcul component a de transparència per llum especular per material.
					                   numMat = numMat +1;
...
// ---- L169- Calcul intensitat final del pixel
   fragmentColor.rgb = Iemissive + Iambient + ILlums;
// ---- L171- Càlcul component a de transparència.
   if (sw_material) { if (numMat != 0) fragmentColor.a = aValue / numMat; else fragmentColor.a = 1.0; // fragmentColor.a = material.diffuse.a;
                    }
	else fragmentColor.a = vertexColor.a;
